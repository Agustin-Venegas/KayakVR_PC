...
Shader "Tazo/OneDirectCloud"
{
	Properties
	{
		...
		//_HF("Horizen Fade", Range(0.01, 5)) = 1
		_E("Edge sharp", Range(0, 10)) = 0.98
		_tile_all("Tile All", Range(0.0, 20.0)) = 1
		...
		Subshader
		{
			...
			//shell
				...
				float4 frag(v2f i) : SV_Target
				{
					...
					float4 t = tex2D(_ProjectUV, (frac(offset * _Time.y * _flow_offset) + i.uv_object.rb * _tileUV * _tile_all));
					fixed4 p = tex2D(_Project, offset * _Time.x + i.uv_object.rb * _tile * _tile_all + t * _flow_strength);
	#ifdef _INVERTC_ON
					fixed4 p_vu = tex2D(_Project, offset.gr * _Time.x + i.uv_object.br * _tile * _tile_all * 1.341 + t * _flow_strength);
					fixed4 p_mask = tex2D(_Project, offset.gr * _Time.x + i.uv_object.br * _tile * _tile_all * 0.687 + t * _flow_strength);
					p = lerp(p_vu,p, p_mask.b);
	#endif
					float2 offset_1 = float2(_tileOffsetX_1 + _tileOffsetX_all, _tileOffsetY_1 + _tileOffsetY_all);
					float4 t_1 = tex2D(_ProjectUV, (offset_1 * _Time.y + frac(offset_1 * _Time.y * _flow_offset_1) + i.uv_object.rb * _tileUV_1 * _tile_all));
					fixed4 p_1 = tex2D(_Project, offset_1 * _Time.x + i.uv_object.rb * _tile_1 * _tile_all + t_1 * _flow_strength_1);

					float2 offset_2 = float2(_tileOffsetX_2 + _tileOffsetX_all, _tileOffsetY_2 + _tileOffsetY_all);
					float4 t_2 = tex2D(_ProjectUV, (offset_2 * _Time.y + frac(offset_2 * _Time.y * _flow_offset_2) + i.uv_object.rb * _tileUV_2 * _tile_all));
					fixed4 p_2 = tex2D(_Project, offset_2 * _Time.x + i.uv_object.rb * _tile_2 * _tile_all + t_2 * _flow_strength_2);
					...
					//aa = saturate(_HF*(aa-0.5)+0.5);
					aa *=aa;
#ifdef _INVERTB_ON
					aa *= aa;
#endif
					cc.a = clamp(   saturate(_D * pow(cc.a * (a),_E))*aa  ,0, _DD);
